...
class BluetoothMeshService(private val context: Context) {
    ...
    // Educational class manager (lazy initialization)
    private val classManager by lazy { ClassManager.getInstance(context) }
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun onFileReceived(fileID: String, fileName: String, fileType: String, fileData: ByteArray, senderPeerID: String, senderName: String) {
                delegate?.didReceiveFile(fileID, fileName, fileType, fileData, senderPeerID, senderName)
            }
            
            override fun onFileTransferProgress(fileID: String, fileName: String, progress: Float) {
                delegate?.didReceiveFileTransferProgress(fileID, fileName, progress)
            }
            
            override fun onFileTransferAckReceived(fileID: String, chunkIndex: Int, receiverPeerID: String) {
                delegate?.didReceiveFileTransferAck(fileID, chunkIndex, receiverPeerID)
            }
            
            override fun onQuizReceived(quizJson: String, senderPeerID: String) {
                delegate?.didReceiveQuiz(quizJson, senderPeerID)
            }
            
            override fun onQuizSubmissionReceived(submissionJson: String, senderPeerID: String) {
                delegate?.didReceiveSubmission(submissionJson, senderPeerID)
            }
            
            override fun onClassAnnouncementReceived(classJson: String, senderPeerID: String) {
                // Parse class announcement and add to discovered classes
                try {
                    val gson = Gson()
                    val classData = gson.fromJson(classJson, Map::class.java) as? Map<String, Any>
                    if (classData != null) {
                        val id = classData["id"] as? String ?: return
                        val name = classData["name"] as? String ?: return
                        val code = classData["code"] as? String ?: return
                        val passwordHash = classData["passwordHash"] as? String ?: return
                        val passwordSalt = classData["passwordSalt"] as? String ?: return
                        val encryptionKey = classData["encryptionKey"] as? String ?: return
                        val teacherId = classData["teacherId"] as? String ?: return
                        val teacherName = classData["teacherName"] as? String ?: return
                        val subject = classData["subject"] as? String ?: ""
                        
                        classManager.addDiscoveredClass(
                            id = id,
                            name = name,
                            code = code,
                        ...
                        Log.d(TAG, "Received class announcement: $name ($code) from $senderPeerID")
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Failed to parse class announcement: ${e.message}")
                }
            }
            
            override fun onClassDiscoveryRequested(classCode: String, requesterPeerID: String) {
                // If we're a teacher with this class, broadcast it
                serviceScope.launch {
                    try {
                        val broadcastData = classManager.getClassBroadcastData(classCode)
                        if (broadcastData != null) {
                            val gson = Gson()
                            val json = gson.toJson(broadcastData)
                            broadcastClassAnnouncement(json.toByteArray(Charsets.UTF_8))
                            Log.d(TAG, "Responded to class discovery request for $classCode from $requesterPeerID")
                        }
                    } catch (e: Exception) {
                        Log.e(TAG, "Failed to respond to class discovery: ${e.message}")
                    }
                }
            }
        }
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleFileTransfer(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleFileTransfer(routed) }
            }
            
            override fun handleFileTransferAck(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleFileTransferAck(routed) }
            }
            
            override fun handleQuizDistribution(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleQuizDistribution(routed) }
            }
            
            override fun handleQuizSubmission(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleQuizSubmission(routed) }
            }
            
            override fun handleClassAnnounce(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleClassAnnounce(routed) }
            }
            
            override fun handleClassDiscovery(routed: RoutedPacket) {
                serviceScope.launch { messageHandler.handleClassDiscovery(routed) }
            }
        }
    ...
    /**
     * Send file transfer chunk via mesh network
     * Uses broadcast for public file sharing or targeted for specific recipients
     */
    fun sendFileTransfer(
        fileID: String,
        fileName: String,
        fileType: String,
        fileSize: Long,
        ...
        serviceScope.launch {
            try {
                val senderName = delegate?.getNickname() ?: "Unknown"
                
                // Create file transfer packet with TLV encoding
                val filePacket = com.bitchat.android.model.FileTransferPacket(
                    fileID = fileID,
                    fileName = fileName,
                    fileType = fileType,
                    fileSize = fileSize,
                ...
                // Create packet (broadcast or targeted)
                val packet = BitchatPacket(
                    version = 1u,
                ...
                val target = recipientPeerID ?: "all peers"
                Log.d(TAG, "📤 Sent file chunk ${chunkIndex + 1}/$totalChunks for $fileName to $target")
                
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send file transfer: ${e.message}")
            }
        }
    }
     ...
     */
    fun sendFileTransferAck(fileID: String, chunkIndex: Int, senderPeerID: String) {
        serviceScope.launch {
            try {
                // Create ACK packet
                ...
                // Encode ACK
                val payload = ackPacket.encode()
                
                // Create packet addressed to sender
                val packet = BitchatPacket(
                    version = 1u,
                ...
                Log.e(TAG, "Failed to send file transfer ACK: ${e.message}")
            }
        }
    }
    
    /**
     * Send quiz distribution via mesh network (broadcast to all students)
     */
    fun sendQuizDistribution(quizData: ByteArray) {
        serviceScope.launch {
            try {
                Log.d(TAG, "📤 Broadcasting quiz distribution (${quizData.size} bytes)")
                
                // Create packet for quiz distribution
                val packet = BitchatPacket(
                    version = 1u,
                    type = MessageType.QUIZ_DISTRIBUTION.value,
                    senderID = hexStringToByteArray(myPeerID),
                ...
                Log.d(TAG, "✅ Quiz distributed successfully via mesh network")
                
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send quiz distribution: ${e.message}")
            }
        }
    }
    
    /**
     * Send quiz submission via mesh network (broadcast to teacher)
     */
    fun sendQuizSubmission(submissionData: ByteArray) {
        serviceScope.launch {
            try {
                Log.d(TAG, "📤 Broadcasting quiz submission (${submissionData.size} bytes)")
                
                // Create packet for quiz submission
                val packet = BitchatPacket(
                    version = 1u,
                    type = MessageType.QUIZ_SUBMISSION.value,
                    senderID = hexStringToByteArray(myPeerID),
                    recipientID = SpecialRecipients.BROADCAST, // Broadcast so teacher receives it
                    timestamp = System.currentTimeMillis().toULong(),
                ...
                Log.d(TAG, "✅ Quiz submission sent successfully via mesh network")
                
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send quiz submission: ${e.message}")
            }
        }
    }
    
    /**
     * Broadcast class announcement via mesh network (teacher only)
     * Students receive this to discover available classes
     */
    fun broadcastClassAnnouncement(classData: ByteArray) {
        serviceScope.launch {
            try {
                Log.d(TAG, "📡 Broadcasting class announcement (${classData.size} bytes)")
                
                // Create packet for class announcement
                val packet = BitchatPacket(
                    version = 1u,
                    type = MessageType.CLASS_ANNOUNCE.value,
                    senderID = hexStringToByteArray(myPeerID),
                    recipientID = SpecialRecipients.BROADCAST,
                    timestamp = System.currentTimeMillis().toULong(),
                    payload = classData,
                ...
                Log.d(TAG, "✅ Class announcement broadcast successfully")
                
            } catch (e: Exception) {
                Log.e(TAG, "Failed to broadcast class announcement: ${e.message}")
            }
        }
    }
    ...
    /**
     * Get verified peers for educational services
     * Exposed for StudentPresenceService to detect online students
     */
    fun getVerifiedPeersForEducation(): Map<String, PeerInfo> {
        return peerManager.getVerifiedPeers()
    }
}
...
interface BluetoothMeshDelegate {
    ...
    fun didReceiveFile(fileID: String, fileName: String, fileType: String, fileData: ByteArray, senderPeerID: String, senderName: String)
    fun didReceiveFileTransferProgress(fileID: String, fileName: String, progress: Float)
    fun didReceiveFileTransferAck(fileID: String, chunkIndex: Int, receiverPeerID: String)
