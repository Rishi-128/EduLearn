...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.FILE_TRANSFER -> handleFileTransfer(routed)
            MessageType.QUIZ_DISTRIBUTION -> handleQuizDistribution(routed)
            MessageType.QUIZ_SUBMISSION -> handleQuizSubmission(routed)
            MessageType.CLASS_ANNOUNCE -> handleClassAnnounce(routed)
            MessageType.CLASS_DISCOVERY -> handleClassDiscovery(routed)
            else -> {
                ...
                if (packetRelayManager.isPacketAddressedToMe(packet)) {
                    when (messageType) {
                        ...
                        MessageType.FILE_TRANSFER_ACK -> handleFileTransferAck(routed)
                        else -> {
                            validPacket = false
     ...
     */
    private suspend fun handleFileTransfer(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing file transfer from ${formatPeerForLog(peerID)}")
        delegate?.handleFileTransfer(routed)
    }
    
    /**
     * Handle file transfer acknowledgment
     */
    private suspend fun handleFileTransferAck(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing file transfer ACK from ${formatPeerForLog(peerID)}")
        delegate?.handleFileTransferAck(routed)
    }
    
    /**
     * Handle quiz distribution message
     */
    private suspend fun handleQuizDistribution(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "📚 Processing quiz distribution from ${formatPeerForLog(peerID)}")
        delegate?.handleQuizDistribution(routed)
    }
    
    private suspend fun handleQuizSubmission(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "📝 Processing quiz submission from ${formatPeerForLog(peerID)}")
        delegate?.handleQuizSubmission(routed)
    }
    
    /**
     * Handle class announcement from teacher
     */
    private suspend fun handleClassAnnounce(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "📚 Processing class announcement from ${formatPeerForLog(peerID)}")
        delegate?.handleClassAnnounce(routed)
    }
    
    /**
     * Handle class discovery request from student
     */
    private suspend fun handleClassDiscovery(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "🔍 Processing class discovery request from ${formatPeerForLog(peerID)}")
        delegate?.handleClassDiscovery(routed)
    }
...
interface PacketProcessorDelegate {
    ...
    fun handleFileTransfer(routed: RoutedPacket)
    fun handleFileTransferAck(routed: RoutedPacket)
    fun handleQuizDistribution(routed: RoutedPacket)
    fun handleQuizSubmission(routed: RoutedPacket)
    fun handleClassAnnounce(routed: RoutedPacket)
    fun handleClassDiscovery(routed: RoutedPacket)
