...
class GeohashRepository(
    ...
    // pubkeyHex(lowercase) -> user role (teacher/student)
    private val geoRoles: MutableMap<String, String> = mutableMapOf()
    ...
    fun cacheRole(pubkeyHex: String, role: String) {
        val lower = pubkeyHex.lowercase()
        val previous = geoRoles[lower]
        geoRoles[lower] = role
        if (previous != role && currentGeohash != null) {
            refreshGeohashPeople()
        }
    }

    fun getCachedRole(pubkeyHex: String): String? = geoRoles[pubkeyHex.lowercase()]
    ...
    fun refreshGeohashPeople() {
        ...
        val people = participants.map { (pubkeyHex, lastSeen) ->
            ...
            val role = getCachedRole(pubkeyHex)
            GeoPerson(
