...
class PermissionManager(private val context: Context) {
    ...
    fun markOnboardingComplete() {
        sharedPrefs.edit().putBoolean(KEY_FIRST_TIME_COMPLETE, true).apply()
    }
    ...
    fun getRequiredPermissions(): List<String> {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            // Android 12+ uses new Bluetooth runtime permissions
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover students and teachers via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow EduLearn to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby students and teachers via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "EduLearn needs this to scan for nearby devices"
            )
        )
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Receive notifications for assignments, quizzes, and private messages",
                    permissions = listOf(Manifest.permission.POST_NOTIFICATIONS),
                    isGranted = isPermissionGranted(Manifest.permission.POST_NOTIFICATIONS),
                    systemDescription = "Allow EduLearn to send you notifications"
                )
            )
        }
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure EduLearn runs reliably in the background and maintains connections with students",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow EduLearn to run without battery restrictions"
                )
            )
        }
