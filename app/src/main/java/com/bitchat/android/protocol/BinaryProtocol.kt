...
enum class MessageType(val value: UByte) {
    ...
    FILE_TRANSFER(0x30u), // File transfer for educational documents
    FILE_TRANSFER_ACK(0x31u), // File transfer acknowledgment
    QUIZ_DISTRIBUTION(0x32u), // Quiz distribution for educational mode
    QUIZ_SUBMISSION(0x33u), // Quiz submission from student to teacher
    CLASS_ANNOUNCE(0x40u), // Class announcement from teacher (code, name, hash, salt, key)
    CLASS_DISCOVERY(0x41u); // Request for class info from student
