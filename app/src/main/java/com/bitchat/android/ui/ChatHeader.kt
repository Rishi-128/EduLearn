...
private fun MainHeader(
    ...
    Row(
        ...
        Row(
            ...
            // Back button to mode selection (if available)
            onBackToModeSelection?.let { backCallback ->
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 14.dp)) {
                ...
                IconButton(
                    onClick = { viewModel.toggleEducationalMode() },
                    modifier = Modifier.size(24.dp)
                ) {
                    Icon(
                        Icons.Filled.School,
                        contentDescription = "Education Mode",
                        modifier = Modifier.size(16.dp),
                        tint = MaterialTheme.colorScheme.primary
                    )
                }
                
                // DISABLED: Location channels / region selection feature is hidden
                // Set to false to show the button again in the future
                val isLocationChannelsFeatureHidden = true
                
                if (!isLocationChannelsFeatureHidden) {
...
private fun LocationChannelsButton(
    ...
    // DISABLED: Internet region selection feature is temporarily disabled
    // Set to false to re-enable the location channels / teleport feature in the future
    val isFeatureDisabled = true
    ...
    // When disabled, show grayed out color
    val displayColor = if (isFeatureDisabled) Color.Gray.copy(alpha = 0.5f) else badgeColor
    
    Button(
        onClick = { if (!isFeatureDisabled) onClick() },
        enabled = !isFeatureDisabled,
        colors = ButtonDefaults.buttonColors(
            containerColor = Color.Transparent,
            contentColor = displayColor,
            disabledContainerColor = Color.Transparent,
            disabledContentColor = displayColor
        ),
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            // Teleportation indicator (like iOS) - hidden when disabled
            if (teleported && !isFeatureDisabled) {
                Spacer(modifier = Modifier.width(2.dp))
                Icon(
