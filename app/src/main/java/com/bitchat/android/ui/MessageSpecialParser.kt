...
object MessageSpecialParser {
     ...
     * Excludes local file references like .pdf, .doc, etc.
     */
    fun findUrls(text: String): List<UrlMatch> {
        ...
        while (matcher.find()) {
            ...
            // Skip local file references - common file extensions that shouldn't be treated as URLs
            val fileExtensions = setOf("pdf", "doc", "docx", "txt", "xls", "xlsx", "ppt", "pptx", "jpg", "jpeg", "png", "gif", "mp4", "avi", "mp3", "wav", "zip", "rar")
            val isFileReference = fileExtensions.any { ext -> 
                token.lowercase().endsWith(".$ext") && !token.contains("://") && !token.startsWith("www.")
            }
        ...
        for (m in bare.findAll(text)) {
            ...
            val fileExtensions = setOf("pdf", "doc", "docx", "txt", "xls", "xlsx", "ppt", "pptx", "jpg", "jpeg", "png", "gif", "mp4", "avi", "mp3", "wav", "zip", "rar")
            val isFileReference = fileExtensions.any { ext -> 
                token.lowercase().endsWith(".$ext") && !token.contains("://")
            }
            if (isFileReference) continue
